...
def get_outline_front_pocket(pocket):
    """Return closed (N,2) polygon outline in cm."""
    pts = pocket['points']
    curves = pocket['curves']
    # opening(upper→lower) → sideseam → bag_bottom(reversed) → inner edge → close
    return np.vstack([
        curves['opening'],                    # pocket_upper → pocket_lower
        [pts['bag_sideseam']],                # → bag_sideseam
        curves['bag_bottom'][::-1][1:],       # → bag_inner_bottom (skip dup sideseam)
        [pts['bag_inner_top']],               # → bag_inner_top
        [pts['pocket_upper']],                # close polygon
    ])
...
def run(measurements_path, output_path, debug=False, units='cm'):
    ...
    return {'front': front, 'front_pocket': pocket}
