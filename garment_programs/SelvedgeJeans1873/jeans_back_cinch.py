...
def get_outline_back_cinch(cinch):
    """Return closed (N,2) polygon outline in cm."""
    pts = cinch['points']
    return np.array([pts['f_tl'], pts['f_tr'], pts['f_br'], pts['f_bl'], pts['f_tl']])


def get_sa_outline_back_cinch(cinch):
    """Return closed SA polygon (cut line) for the back cinch in cm."""
    pts = cinch['points']
    return np.array([pts['sa_tl'], pts['sa_tr'], pts['sa_br'], pts['sa_bl'], pts['sa_tl']])
